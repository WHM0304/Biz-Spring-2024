...
public class MybatisContextConfig {
	...
	public SqlSessionFactoryBean sessionFactoryBean() {
		...
		// ~src/main/resoutce 폴더에 mapper/*-mapper.xml 을 찾아라
...
//		PlatformTransactionManager manager = new DataSourceTransactionManager();
		 ...
		 * 	그런데 Platform... 에는 DataSource 를 주입하는 method 가 정의 되지 않았다
		 * 	결국 Platform... 에는 없지만 DataSource...Manager 에만 있는 setDataSource()
		 * 	method 를 사용하여야 한다.
		 * 	이럴때 Platform... 에는 없지만, DataSource...Manager 에만 있는 setDataSource()
		 * 	method 를 사용하기 위해서는 DataSource...Manager type 으로 
		 * 	Casting(강제형변환)을 수행해야 한다.
		 */
//		((DataSourceTransctionManager)manager).setDataSource(this.ds());
		return manager;
	}
