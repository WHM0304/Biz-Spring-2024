...
public class UserServiceImpl implements UserService{
	...
	/*
	 * 회원가입 절차
	 * 1. 기존의 가입된 회원이 있는지 확인
	 * 2. 있으면 새로 추가(가입)되는 회원은 일반 사용자
	 * 3. 없으면 새로 추가(가입)되는 회원은 ADMIN 이며 일반 사용자 ( 2개를 가진다.)
	 */
	@Transactional
	@Override
	public UserVO createUser(UserVO createUserVO) {
		...
		if(userList == null || userList.size() <= 0) {
			roles.add(RoleVO.builder().r_username(username)
	...
	/*
	 * Controller 에서 Dao 를 통하여 User 정보를 조회할수도 있다
