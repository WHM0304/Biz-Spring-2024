...
public class AuthProviderImpl implements AuthenticationProvider{
	...
	/*
	 * @Autowired
	 * 기본생성자를 통하여 UserDao type 의 userDao 객체를 주입 선언
	 * Autowired 를 사용하여 생성자 주입을 하게되면 
	 * 		userDao 객체에 final 을 사용할수 없다.
	 * 클래스영역, 필드영역 선언되는 객체(클래스를 이용하여 선언한 변수들)는
	 * 		가급적 final 을 부착시키는 것이 메모리 관리에서 좋다
	 * final 을 부착하면 사용이 종료된 객체들이 
	 * 자동으로 GC(Garbage Collection , 객체 메모리 제거) 가 된다.
	 * final 을 부착하지 않으면 GC 이 지연되는 현상이 있다
	...
	public Authentication authenticate(Authentication authentication) throws AuthenticationException {
		...
		if(userVO == null) {
			...
			throw new UsernameNotFoundException(String.format
					("USERNAME 확인 : %s는 없음", username));
		}
		
		//로그인할때 입력한 비번(password)과
		// DB에 저장된 암호화된 비번(userVO.getPassword()) 를 비교하라
