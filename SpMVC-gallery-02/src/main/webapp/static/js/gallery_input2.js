document.addEventListener("DOMContentLoaded", () => {
  ...
  base64 제약 사항
      파일크기가 상당히 크다
      jpeg 는 그대로 다소 양호 하나, png 나 백터 타입 파일은 여러가지 이슈가 있다.
      파일의 크기문제로 업로드, DB 저장등에서 문제를 일으킬수 있다.
  다만, DB에 파일을 저장하므로써 별도의 이미지를 보관하는 방식에 비해 다소 유리한 점도 있다.
  ...
  const ecodeImageFileAsBase64 = async (image) => {
    return new Promise((resolve, _) => {
      ...
      reader.onloadend = () => {
        ...
        //   // canvas 에 그려진 이미지를 jpeg 로 변환하고 크기를 0.2배
        //   let point = 0.5;
