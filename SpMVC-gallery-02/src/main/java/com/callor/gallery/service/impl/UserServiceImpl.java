...
public class UserServiceImpl implements UserService{
	 ...
	 * UserDao.insert()를 통하여 회원가입 하기
	 * 회원가입을 할때 사용자의 권한을 어떻게 부여할 것인가
	 * 
	 ...
	 * 만약 sql 명령을 실행하는 과정에서 exception 이 발생하면
	 * 이를 method 에서 자체적으로 try...catch 로 처리를 해버리면
	 * 현재 method 는 정상적으로 실행이 된것으로 인식되어 버린다
	...
	public int create_user(UserVO userVO)  throws DataAccessException{
		...
		if(resultVO == null || resultVO.size() <1) {
			grantList.add(
					RoleVO.builder().r_username(userVO.getUsername())
